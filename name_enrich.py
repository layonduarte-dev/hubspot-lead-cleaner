...
Acumula as estatísticas da operação e gera:
  - um texto de log (vai para a aba "Log" do arquivo de saída);
  - um resumo amigável para o terminal.

Manter o relatório separado do pipeline deixa a orquestração limpa e
facilita testar a lógica de limpeza sem se preocupar com formatação.
"""
...
@dataclass
class Relatorio:
    """Coletor de métricas do pipeline."""
    ...
    def gerar_log(self) -> str:
        """Texto detalhado para a aba Log do arquivo de saída."""
        linhas = [
        ...
        if self.colunas_preservadas:
            linhas.append("Colunas preservadas (intocadas):")
            for c in self.colunas_preservadas:
                linhas.append(f"  - {c}")

        if self.dominios_corrigidos:
            linhas.append("")
            linhas.append("Exemplos de domínios corrigidos (antes -> depois):")
    ...
    def resumo_terminal(self) -> str:
        """Versão curta e visual para o terminal."""
        return (
            "\n✅ Higienização concluída!\n"
            f"   Entrada........: {self.linhas_entrada} linhas\n"
            f"   Sem e-mail.....: {self.linhas_sem_email} removidas\n"
            f"   Inválidos......: {self.sintaxe_invalida} sintaxe + "
            f"{self.lixo_removido} lixo\n"
            ...
            f"   Cargos.........: {self.cargos_mapeados} mapeados "
            f"({self.cargos_outros} em 'Outros')\n"
